...
from multiModel import loadAndPreprocessData, createMultimodalSequences, MultimodalDataset, preloadHeatmaps, CnnEncoder, NbaMultimodal
...
def train_and_simulate():
    ...
    print(f"Avg Implied Std: {avg_std_implied:.2f}")
    print(f"Actual Std: {actual_std:.2f}")
    print(f"Capture Ratio: {avg_std_implied/actual_std:.2f}")

    # House Bias Analysis & Metrics
    ...
    for t_idx, t_name in enumerate(target_names):
        ...
        line_str = f"{t_name} | Bias: {house_bias:.2f} (Pos=Underest) | MAE: {house_mae:.2f} | StdErr: {house_std_resid:.2f}"
        print(line_str)
